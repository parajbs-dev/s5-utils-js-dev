 ...
 */
export async function decryptFileXchacha20(chunk: Uint8Array, key: Uint8Array, chunkIndex: number): Promise<Uint8Array> {
  await sodium.ready;
  ...
  const decryptedChunk = sodium.crypto_aead_xchacha20poly1305_ietf_decrypt(
    null,
    chunk,
    null,
    nonce,
 ...
 */
export async function hashStream(stream: ReadableStream<Uint8Array>): Promise<Uint8Array> {
...
export async function decryptStream(
  b3hashStream: ReadableStream<Uint8Array>,
  clonedStream: ReadableStream<Uint8Array>,
  durlEncryptionMetadataHash: Buffer,
  key: Uint8Array,
  chunkSize: number
): Promise<{ blob: Blob }> {
  await sodium.ready;
...
export async function decryptFile(
  ...
  try {
    ...
    if (b3hashFileStream && encryptedFileStream) {
      try {
        const decryptedBlob: DecryptedBlobObject = await decryptStream(
          b3hashFileStream,
          encryptedFileStream,
          durlEncryptionMetadataHash,
          decryptionKey,
          262160
        );
        console.debug('\nDecryption end');
 ...
 */
export function compareB3hash(b3hash1: Buffer, b3hash2: Buffer): number {
 ...
 */
export async function getEncryptedFileUrl(encryptedCid: string, locationsHost: string): Promise<EncryptedFileUrlResponse> {
  let uCid;
  let bytes;
  let url0;
  let hash_b64;
 ...
 */
export function decodeBase64URL2(input: string): Buffer {
 ...
 */
export function decodeEndian(bytes: Buffer): number {
  let value = 0;
 ...
 */
export function hashToBase64UrlNoPadding(hashBytes: Uint8Array | Buffer): string {
...
export const streamingUrlCache: Record<string, string[]> = {};
 ...
 */
export async function getStreamingLocation(hash: string, types: string, locationsHost: string): Promise<string[]> {
 ...
 */
export async function getS5LocationsApiHost(portalUrl: string) {
...
export async function checkOptsLocationsAPI(opts: any): Promise<string> {
