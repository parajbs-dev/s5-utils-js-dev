...
import { generateCIDFromMHash, calculateB3hashFromFile, generateMHashFromB3hash, encryptionAlgorithmXChaCha20Poly1305, cidTypeEncrypted, } from "./index";
 ...
 */
export async function generateKeyXchacha20() {
    await sodium.ready;
 ...
 */
export async function encryptFileXchacha20(inputFile, encryptKey) {
    await sodium.ready;
 ...
 */
export async function getXchacha20KeyAndClear() {
    if (!cryptoKey) {
 ...
 */
export async function encryptFileV1(file, encryptKey) {
 ...
 */
export async function generateEncryptedCIDFromMHash(encryptedMHash, file) {
    ...
    const encryptedCidBytes = createEncryptedCid(cidTypeEncrypted, encryptionAlgorithm, chunkSizeAsPowerOf2, encryptedMHash, encryptedKey, padding, cid);
