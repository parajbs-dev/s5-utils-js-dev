...
exports.checkOptsLocationsAPI = exports.getS5LocationsApiHost = exports.getStreamingLocation = exports.streamingUrlCache = exports._base64ToUint8Array = exports.hashToBase64UrlNoPadding = exports.decodeEndian = exports.decodeBase64URL2 = exports.getEncryptedFileUrl = exports.compareB3hash = exports.decryptFile = exports.decryptStream = exports.hashStream = exports.decryptFileXchacha20 = void 0;
 ...
 */
async function decryptFileXchacha20(chunk, key, chunkIndex) {
    await libsodium_wrappers_1.default.ready;
    const nonce = new Uint8Array(libsodium_wrappers_1.default.crypto_aead_xchacha20poly1305_ietf_NPUBBYTES);
    const indexBytes = new Uint8Array(4);
    indexBytes.set(new Uint8Array(new Uint32Array([chunkIndex]).buffer), 0);
    nonce.set(indexBytes, 0);
    const decryptedChunk = libsodium_wrappers_1.default.crypto_aead_xchacha20poly1305_ietf_decrypt(null, chunk, null, nonce, key);
    if (!decryptedChunk) {
        throw new Error('Decryption error.');
    }
    return decryptedChunk;
}
exports.decryptFileXchacha20 = decryptFileXchacha20;
/**
 ...
 */
async function decryptStream(b3hashStream, clonedStream, durlEncryptionMetadataHash, key, chunkSize) {
    await libsodium_wrappers_1.default.ready;
    let decryptedBlob;
    ...
    try {
        ...
        const compareB3hashes = await compareB3hash(durlEncryptionMetadataHash, buffer_1.Buffer.from(hash));
        if (compareB3hashes === 0) {
            ...
            libsodium_wrappers_1.default.memzero(currentChunk);
            libsodium_wrappers_1.default.memzero(key);
            libsodium_wrappers_1.default.memzero(durlEncryptionMetadataHash);
            chunkIndex = 0;
            chunks = [];
            return decryptedBlob;
        }
 ...
 */
async function decryptFile(response, durlEncryptionMetadataHash, decryptedFilePath, decryptionKey, chunkIndex) {
    try {
        await libsodium_wrappers_1.default.ready;
        if (chunkIndex) {
...
function compareB3hash(b3hash1, b3hash2) {
    ...
    const buffer2 = buffer_1.Buffer.concat([buffer_1.Buffer.from([31]), buffer_1.Buffer.from(b3hash2)]);
    // Compare buffer1 with buffer2 and store the result
    const compareB3hashes = buffer_1.Buffer.compare(buffer1, buffer2);
    // Return the comparison result
    return compareB3hashes;
}
exports.compareB3hash = compareB3hash;
/**
 ...
 */
async function getEncryptedFileUrl(encryptedCid, locationsHost) {
    let uCid;
    let bytes;
    let url0;
    let hash_b64;
    ...
    if (encryptedCid[0] === 'u') {
        ...
        bytes = (0, basetools_1.decodeBase64URL)(cid.substr(1));
        if (bytes[0] == 0xae) {
            if (bytes[1] != 0xa6) {
            ...
            uCid = 'u' + (0, blake3tools_1.convertMHashToB64url)(bytes);
...
exports.getEncryptedFileUrl = getEncryptedFileUrl;
/**
...
function decodeBase64URL2(input) {
    ...
    return buffer_1.Buffer.from(paddedBase64, 'base64');
}
exports.decodeBase64URL2 = decodeBase64URL2;
/**
...
exports.decodeEndian = decodeEndian;
/**
...
function hashToBase64UrlNoPadding(hashBytes) {
    ...
    const base64 = buffer_1.Buffer.from(hashBytes).toString('base64');
...
exports.hashToBase64UrlNoPadding = hashToBase64UrlNoPadding;
/**
...
exports._base64ToUint8Array = _base64ToUint8Array;
exports.streamingUrlCache = {};
/**
 ...
 */
async function getStreamingLocation(hash, types, locationsHost) {
    // Check if the streaming locations are already cached
    const val = exports.streamingUrlCache[hash];
    if (val !== undefined) {
    ...
    exports.streamingUrlCache[hash] = parts;
    return parts;
}
exports.getStreamingLocation = getStreamingLocation;
/**
 ...
 */
async function getS5LocationsApiHost(portalUrl) {
...
exports.getS5LocationsApiHost = getS5LocationsApiHost;
/**
...
async function checkOptsLocationsAPI(opts) {
...
exports.checkOptsLocationsAPI = checkOptsLocationsAPI;
